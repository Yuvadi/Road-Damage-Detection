...
def do_pruning(modelpath, savepath):
    ...
    pruning.get_threshold(yolo.model, 0.8)  # 0.9 is the pruning rate

    ### 1. Prune Bottleneck in C3k2
    for name, m in yolo.model.named_modules():
        if isinstance(m, Bottleneck):
            pruning.prune_conv(m.cv1, m.cv2)

    ### 2. Prune convolutional layers between different modules
    seq = yolo.model.model
    for i in [3, 5, 7, 8]:
        pruning.prune(seq[i], seq[i + 1])

    ### 3. Prune detection head
    detect: Detect = seq[-1]
    proto = getattr(detect, 'proto', None)  # Safely get the proto attribute if it exists
    ...
    ### 4. Set model gradients and save
...
if __name__ == "__main__":
    ...
    savepath = "runs/obb/train8/weights/last_prune_80.pt"
